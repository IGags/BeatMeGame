...
{
    public class FFT //TODO: сделать мод записи и чтения детекшена, написать тесты
    {
        ...
        private readonly List<double[]> FFTBuffer = new List<double[]>();
        private readonly List<bool> beatBuffer = new List<bool>();
        ...
        public double[] GetFFTData(TimeSpan trackPosition)
        {
            ...
            return FFTBuffer.Count == 0 ? new double[FFTSize / 2] : FFTBuffer[ChooseElement(trackPosition)];
        }
        ...
        public List<bool> GetBeatSecond(TimeSpan trackPosition)
        {
            beatBuffer.Clear();
            GetFFTData(trackPosition);
            ComputeBeat();
        ...
        public void ComputeBeat()
        {
            ...
            highEdge = highEdge == 512 ? 511 : highEdge;
